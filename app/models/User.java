 ...
 *
 * @version 1.0 1st June 2015 * @author Shekhar Ullah
 */
@Entity
public class User implements Subject {
	...
	@Embedded
	private DoctorsRate rate=new DoctorsRate();

	public DoctorsRate getRate() {
		return rate;
	}

	public void setRate(DoctorsRate rate) {
			this.rate = rate;
	}
	...
	private static Query<User> getUsernamePasswordAuthUserFind(
			final UsernamePasswordAuthUser identity) {
		return getEmailUserFind(identity.getEmail()).filter(
	...
	private static Query<User> getEmailUserFind(final String email) {
		return MorphiaObject.datastore.createQuery(User.class)
				.filter("active", true).filter("email", email);
	}
